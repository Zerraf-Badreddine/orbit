...
export async function sendVerificationEmailWrap(options: VerificationEmail) {
  ...
  if (!parsed.success) {
    throw new Error(
      `Invalid verification email option ${parsed.error.message}`,
    );
  }
...
function createResetPasswordTemplate(url: String, token: String) {
  return `
        <!DOCTYPE html>
        <html>
        <body style="font-family: Arial, sans-serif; padding: 20px;">
...
// Reset password email have same options as verification email we use same infered type
export async function sendResetPasswordEmail(options: VerificationEmail) {}
